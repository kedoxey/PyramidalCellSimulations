...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 Ih /home/kedoxey/CRCNS/PyramidalCellSimulations/models/local/Birgiolas2020/Mechanisms/Ih.mod\n");
...
static int  _f_rates (  double _lv ) {
   rinf = 1.0 / ( 1.0 + exp ( ( _lv / 1.0 + 84.1 ) / 10.2 ) ) ;
   rexp = 1.0 - exp ( - dt / 1.0 / ( tau_r ) ) ;
   tau_r = 100.0 + 1.0 / ( exp ( - 17.9 - 0.116 * _lv / 1.0 ) + exp ( - 1.84 + 0.09 * _lv / 1.0 ) ) ;
   tau_r = tau_r / qt ;
    return 0; }
...
_deriv1_advance = 0;
 if(error){fprintf(stderr,"at line 42 in file Ih.mod:\n    SOLVE deriv METHOD derivimplicit\n"); nrn_complain(_p); abort_run(error);}
    if (secondorder) {
    int _i;
    for (_i = 0; _i < 1; ++_i) {
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/kedoxey/CRCNS/PyramidalCellSimulations/models/local/Birgiolas2020/Mechanisms/Ih.mod";
static const char* nmodl_file_text = 
  ...
  "\n"
  "	rinf = 1/(1 + exp((v/1(mV)+84.1)/10.2))\n"
  "	rexp = 1 - exp(-dt/1(ms)/(tau_r))\n"
  "	tau_r = 100 + 1/(exp(-17.9-0.116*v/1(mV))+exp(-1.84+0.09*v/1(mV)))\n"
  "	tau_r = tau_r / qt\n"
  "}\n"
  " \n"
  "UNITSON\n"
